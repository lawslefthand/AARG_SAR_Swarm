...
def takeoff(mav_connection, takeoff_altitude: float, lat , lon, tgt_sys_id , tgt_comp_id=1 ):
...
def connect(UDP1,UDP2,sysid1,sysid2):
    mav_connection1 = connect_to_sysid(f'udp:localhost:{UDP1}',sysid1)
    mav_connection2 = connect_to_sysid(f'udp:localhost:{UDP2}',sysid2)

    return mav_connection1, mav_connection2



def print_lat_lon(connection1, connection2):
    while True:
        time.sleep(4)
        msg1 = connection1.recv_match(type='GLOBAL_POSITION_INT', blocking=True)
        lat1 = msg1.lat / 10 ** 7
        lon1 = msg1.lon / 10 ** 7

        msg2 = connection2.recv_match(type='GLOBAL_POSITION_INT', blocking=True)
        lat2 = msg2.lat / 10 ** 7
        lon2 = msg2.lon / 10 ** 7

        distance = haversine(lat1, lon1, lat2, lon2)
        print(f"Distance between drones: {distance*1000} ")
...
def main():
    ...
    contact_1, contact_2 = connect(14540, 14560, 1, 1)

    #takeoff threads for drones

    Drone_1 = threading.Thread(target=takeoff,args=(contact_1,10.00, -35.3627491, 149.1651857,1,1))
    Drone_2 = threading.Thread(target=takeoff,args=(contact_2,10.00, -35.3628476, 149.1656658,1,1))
    lat_lon_thread = threading.Thread(target=print_lat_lon, args=(contact_1, contact_2))

    #takeoff process begins
    Drone_1.start()
    Drone_2.start()
    lat_lon_thread.start()
